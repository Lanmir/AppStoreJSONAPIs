...
//  AppStoreJSONAPIs
//
//  Created by Varshaa vasundra sivakumar on 22/11/24.
//
...
class CompositionalController: UICollectionViewController {
    ...
    init(){
        ...
        let layout = UICollectionViewCompositionalLayout { sectionNumber, _ in
            ...
            if sectionNumber == 0{
                ...
                let kind = UICollectionView.elementKindSectionHeader // collectionView header property
    ...
    override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if indexPath.section == 1 {
            title = free?.feed.title
        } else {
            title = paid?.feed.title
        }
    ...
    private func fetchApps() {
        // Slow synchronous fetching one after other
...
//        } else if section == 1 {
//            return free?.feed.results.count ?? 0
//        } else {
//            return paid?.feed.results.count ?? 0
//        }
//
//    }
...
//            appId = free?.feed.results[indexPath.item].id ?? ""
//        } else {
//            appId = paid?.feed.results[indexPath.item].id ?? ""
//        }
...
//    }
    ...
    class CompositionalHeader: UICollectionReusableView{
        ...
        let label = UILabel(text: "Free Apps", font: .boldSystemFont(ofSize: 32))
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = .init(title: "Fetch Trending", style: .plain, target: self, action: #selector(handleFetchTrending))
...
//        fetchApps()
    ...
    @objc fileprivate func handleRefresh() {
        ...
        snapshot.deleteSections([.apps])
        diffableDatasource.apply(snapshot)
        self.navigationItem.rightBarButtonItem?.isEnabled = true
    }
     
    @objc fileprivate func handleFetchTrending() {
        Service.shared.fetchAppGroup(urlString: "https://rss.applemarketingtools.com/api/v2/in/apps/top-free/25/apps.json") { appGroup, err in
            
            var snapshot = self.diffableDatasource.snapshot()
            snapshot.insertSections([.apps], afterSection: .topSocial)
            snapshot.appendItems(appGroup?.feed.results ?? [], toSection: .apps)
    ...
    lazy var diffableDatasource:  UICollectionViewDiffableDataSource<AppSection,AnyHashable> = .init(collectionView: self.collectionView) { (collectionView, indexPath, object) -> UICollectionViewCell? in
        ...
        return nil
    }  // didn't work when set inside func why?
    
    @objc func handleGet(button: UIView) {
//        print(123)
        var superview = button.superview
        //i want to reach the parent cell of the button
        while superview != nil {
...
//        collectionView.dataSource = diffableDatasource
        ...
        diffableDatasource.supplementaryViewProvider = .some({ collectionView, kind, indexPath in
            ...
            if section  == .paid {
                ...
                header.label.text = "Trending Apps"
            }
            return header
        })
        ...
        Service.shared.fetchSocialApps { (socialApps, err) in
            ...
            Service.shared.fetchTopFree { appGroup, err in
                ...
                Service.shared.fetchTopPaid { paidGroup, err in
                    var snapshot = self.diffableDatasource.snapshot()
                    
                    //top social
                    snapshot.appendSections([.topSocial, .paid, .free])
                    snapshot.appendItems(socialApps ?? [], toSection: .topSocial)
                    
                    
                    //free
                    let objects = appGroup?.feed.results ?? []
                    snapshot.appendItems(objects, toSection: .free)
                    snapshot.appendItems(paidGroup?.feed.results ?? [], toSection: .paid)
